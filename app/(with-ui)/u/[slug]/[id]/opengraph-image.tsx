...
export default async function Image({
  ...
  try {
    ...
    const { name, description, thumbnail } = characterRes;

    if (!thumbnail) {
      return new Response("Thumbnail is required", { status: 400 });
    }

    const convertedTumbnail = getPublicUrl(thumbnail);

    // SSRF mitigation: Only allow thumbnail URLs from approved hosts
    if (
      !convertedTumbnail.startsWith("https://") ||
      !isAllowedExternalUrl(convertedTumbnail)
    ) {
      return new Response("Untrusted thumbnail URL", { status: 400 });
    }

    const imageRes = await fetch(convertedTumbnail);
